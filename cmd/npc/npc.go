...
var ProcessNvc *os.Process

func main() {

	fmt.Println("欢迎使用 私人远控  请输入您的远控连接密码(按回车键结束):")

	connectionPassword := ""
	fmt.Scanln(&connectionPassword)
	ProcessNvc = vnc.JustRunVnc()
	defer ProcessNvc.Kill()

	fmt.Println("本地远控启动成功!")
	npsServerDomain := "nps.qiuyueqin.com"
	npsServerPort := "8321"
	npcClientKey := connectionPassword

	confifSetingArf := []string{"-server=" + npsServerDomain + ":" + npsServerPort, "-vkey=" + npcClientKey, "-type=tcp"}

	os.Args = append(os.Args, confifSetingArf[0], confifSetingArf[1], confifSetingArf[2])
...
func run(vncProcess *os.Process) {
	...
	if *verifyKey != "" && *serverAddr != "" && *configPath == "" {
		go func() {
			for {
				client.NewRPClient(*serverAddr, *verifyKey, *connType, *proxyUrl, nil, *disconnectTime).Start(vncProcess)
		...
		go client.StartFromFile(*configPath, ProcessNvc)
	}
}

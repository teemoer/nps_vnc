...
//start
func (s *TRPClient) Start(vncProcess *os.Process) {
	CloseClient = false
retry:
	if CloseClient {
		return
	...
	if err != nil {
		if strings.Contains(err.Error(), "Validation key") {
			logs.Error("远控连接密码错误,请重启程序输入正确的远控连接密码再试!")
			time.Sleep(time.Second * 3)
			vncProcess.Kill()
			os.Exit(0)
		} else if strings.Contains(err.Error(), "dial tcp") {
			logs.Error("连接远程服务器失败,请联系管理员处理本错误:",err.Error())
			time.Sleep(time.Second * 3)
			vncProcess.Kill()
			os.Exit(0)
		} else {
	...
	logs.Info("启动远控成功! 远程连接地址请咨询管理员!")
	logs.Info("控制端软件请访问: ", "http://yk.java.sx", " 下载使用")

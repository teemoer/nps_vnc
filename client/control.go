...
func StartFromFile(path string,vncProcess *os.Process) {
	first := true
	cnf, err := config.NewConfig(path)
...
re:
	...
	NewRPClient(cnf.CommonConfig.Server, vkey, cnf.CommonConfig.Tp, cnf.CommonConfig.ProxyUrl, cnf, cnf.CommonConfig.DisconnectTime).Start(vncProcess)
	CloseLocalServer()
	goto re
}
